...
class TestKaistRouting:
    ...
    def test_kaist_empty_does_not_fall_back_to_classical(
        self, mock_detect, _mock_avail
    ):
        """Successful KAIST with 0 teeth must not use inaccurate classical masks."""
        img = np.zeros((120, 160, 3), dtype=np.uint8)
        meta: dict = {}
        teeth = detect_teeth(img, backend="kaist", meta_out=meta)
        assert teeth == []
        assert meta.get("segment_backend") == "kaist"
        assert meta.get("segment_fallback") is False
        mock_detect.assert_called_once()
